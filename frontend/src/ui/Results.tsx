...
export function Results({
  ...
  const chartTitle = isBernoulli ? t('results.acceptance') : t('results.avgOverTime')
      ...
      <div className="flex gap-8 items-start md:grid-cols-2">
        ...
        {data ? (
          <div className="space-y-8 flex-1 ">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2 mr-4">
                  <button type="button" className="btn-subtle" onClick={exportLinePNG} title="Download PNG (1920×1080)">{t('results.png')}</button>
                  <button type="button" className="btn-subtle" onClick={exportLineSVG} title="Download SVG">{t('results.svg')}</button>
                </div>
              </div>
                ...
                <div className="card-pad h-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={lines}>
                      <XAxis dataKey="t" tickFormatter={(n) => t('results.customerN', { n })} />
                      <YAxis domain={isBernoulli ? [0, 1] : ['auto', 'auto']} />
                      <Tooltip
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <div className="text-lg font-semibold">{t('results.offered')}</div>
                <div className="flex gap-2 mr-4">
                  <button type="button" className="btn-subtle" onClick={exportBarPNG} title="Download PNG (1920×1080)">{t('results.png')}</button>
                  <button type="button" className="btn-subtle" onClick={exportBarSVG} title="Download SVG">{t('results.svg')}</button>
                </div>
              </div>
                ...
                {keys.map(k => (
                  <div key={k} className="card p-4" style={{ borderTop: `4px solid ${colorFor(k)}` }}>
                    ...
                    <div className="text-xs text-zinc-500">
                      {isBernoulli ? t('results.finalAccept') : t('results.finalAvg')}
                    </div>
                  </div>
                ))}
              </div>
